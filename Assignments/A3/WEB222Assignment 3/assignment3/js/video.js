...
var movie= document.querySelector("#film");


var start = "<video"; //keeping the video tag open to accomodate the dimensions and controls 
var end = "</video>";

for (var i = 0; i < video.source.length; i++) {
    start += " width='" + video.width + "' height='" + video.height + "' controls>";
        start += "<source src='" + video.source[i].src + "' type='" + video.source[i].type + "' ";
            start = start + end; // merging the 2 tags
            movie.innerHTML = start;
    }
